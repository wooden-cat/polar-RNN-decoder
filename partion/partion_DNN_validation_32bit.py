...
ber_differ_snr = np.zeros(validation_stop_snr - validation_start_snr + 1, dtype=np.float32)

for validation_snr in range(validation_start_snr, validation_stop_snr + 1):
    A = polar_design_awgn_AA(code_n, code_k, validation_snr)
...
# 画图 训练次数影响误码率
plt.plot(range(validation_start_snr, validation_stop_snr + 1), ber_differ_snr, 'ro')
plt.grid(True)
legend = []
...
plt.xlabel('SNR')
plt.ylabel('BER')
plt.title('BER of different SNR')
plt.show()
